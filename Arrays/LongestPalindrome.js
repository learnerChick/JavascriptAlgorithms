/*
    ...
    1) First, get the length of string.  Length <= 0 is invalid.  Length === 1 is considered a palindrome,
        so return that string.
    2) Now, set the longest palindrome as a substring from 0 to 1.
    3) Iterate through the string from 0 to it's length.  In this loop, we shall be calling out to a
        helper function that will check for the existence of palindromes from index to outwards.
        (this is for odd length string)
        a) In the helper function, in a while loop check if
            begin index is >= 0 && end index is <= str.length - 1 && str[begin] === str[end].
            If it passes, decrement begin by one and and increment end by one.
        b) After exiting while loop, return the part of the string we determined was a substring
            by getting substring(begin+1, end).
...
function longestPalindrome(str){
    ...
    for(var i = 0; i < length; i++){
        ...
        //need this below for even length strings.  If we don't do below, we won't be able to check the entire string
