define(["Node", "../Queue/Queue.RequireJS"], function(Node, Queue){
  ...
  BinarySearchTree.prototype.getSize = function(){
    ...
    //get size via a count in the node of each tree
    ...
    //here, we get the size when traversing the tree, without any values on the node
    BinarySearchTree.prototype.getTreeSize = function(){
      return this.getSizeViaTraversal(this.root);

    }


    BinarySearchTree.prototype.getSizeViaTraversal = function(node){
      if(node === null){
        return 0
      }
      else{
        return this.getSizeViaTraversal(node.left) + 1 + this.getSizeViaTraversal(node.right);
      }
    }
